...
import os
import sys
# use dff_config to import config parameter(replace config in original project)
from config.dff_config import config, update_config

def parse_args():
    parser = argparse.ArgumentParser(description='Train deeplab dff network')
    # general
...
# sys.path.insert(0,'/home/syzhang/Software/mxnet/python')
import shutil
import numpy as np
import mxnet as mx
...
from core.loader import TrainDataLoaderVideo # for cityscape video
...
def train_net(args, ctx, pretrained,pretrained_flow, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    # print sym.infer_shape(data=(1,3,768,1024), data_ref=(1,3,768,1024))
    # setup multi-gpu
    batch_size = len(ctx)
    ...
    print("======== Step:1,Starting to load_gt_segdb =========")
    segdbs = [load_gt_segdb(config.dataset.dataset, image_set, config.dataset.root_path, config.dataset.dataset_path,
                            result_path=final_output_path, flip=config.TRAIN.FLIP, video=config.TRAIN.VIDEO)
              for image_set in image_sets]
    segdb = merge_segdb(segdbs)

    # load training data
    train_data = TrainDataLoaderVideo(sym, segdb, config, batch_size=input_batch_size, crop_height=config.TRAIN.CROP_HEIGHT, crop_width=config.TRAIN.CROP_WIDTH,
    ...
    max_data_shape = [('data', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in max_scale]), max([v[1] for v in max_scale]))),
                      ('data_ref', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in max_scale]),max([v[1] for v in max_scale]))),
                      ('eq_flag', (1,))]
