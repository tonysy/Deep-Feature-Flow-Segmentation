...
class resnet_v1_101_deeplab_dcn_duc(Symbol):
    def __init__(self):
        """
    ...
    def get_train_symbol(self, num_classes, aspp_num = 4, aspp_stride=6, cell_cap = 256, exp='cityscapes'):
        """
        get duc symbol for training
        :param num_classes: num of classes
        :return: the symbol for training 
        """
        data = mx.symbol.Variable(name='data')
        ...
        for i in range(aspp_num):
            ...
            dilate = pad 
            conv_aspp = mx.symbol.Convolution(data=conv_feat, num_filter=cell_cap * num_classes, 
                                            kernel=(3,3), pad=pad, dilate=dilate, 
                                            name=('fc1_%s_c%d' %(exp, i)), 
                                            workspace=self.workspace)
            aspp_list.append(conv_aspp)
        
        summ = mx.symbol.ElementWiseSum(*aspp_list, name=('fc1_%s' % exp))
        cls_score_reshape = mx.symbol.Reshape(data=summ, shape=(0, num_classes, -1), name='cls_score_reshape')

        softmax = mx.symbol.SoftmaxOutput(data=cls_score_reshape, label=seg_cls_gt,
                                        normalization='valid', multi_output=True, 
                                        use_ignore=True, ignore_label=255, name='softmax' )
