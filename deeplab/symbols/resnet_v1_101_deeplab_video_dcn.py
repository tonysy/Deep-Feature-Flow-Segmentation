# --------------------------------------------------------
# Deformable Convolutional Networks For Video Semantic Segmentation
...
class resnet_v1_101_deeplab_video_dcn(Symbol):
    def __init__(self):
        """
    ...
    def get_train_symbol_video(self, num_classes):
        """
        ...
        seg_cls_gt = mx.sym.Variable(name='label')


        # shared convolutional layers
        conv_feat = self.get_resnet_conv(data_ref)
        ...
        # flownet warp get feature
        flow, scale_map = self.get_flownet(data, data_ref)
        flow_grid = mx.sym.GridGenerator(data=flow, transform_type='warp', name="flow_grid")
        warping_relu_fc6 = mx.sym.BilinearSampler(data=relu_fc6, grid=flow_grid, name='warping_relu_fc6')
        warping_relu_fc6= warping_relu_fc6 * scale_map
        select_relu_fc6 = mx.sym.take(mx.sym.Concat(*[warping_relu_fc6, relu_fc6], dim=0), eq_flag)

        score_bias = mx.symbol.Variable('score_bias', lr_mult=2.0)
        score_weight = mx.symbol.Variable('score_weight', lr_mult=1.0)

        score = mx.symbol.Convolution(data=select_relu_fc6, kernel=(1, 1), pad=(0, 0), num_filter=num_classes, name="score",
    ...
    def get_test_symbol_video(self, num_classes):
        """
        get symbol for testing
        :param num_classes: num of classes
        :return: the symbol for tesing
        """
        data = mx.symbol.Variable(name='data')
        data_ref = mx.symbol.Variable(name='data_ref')
        eq_flag = mx.sym.Variable(name='eq_flag')
        ...
        score = mx.symbol.Convolution(
            data=select_relu_fc6, kernel=(1, 1), pad=(0, 0), num_filter=num_classes, name="score", bias=score_bias,
        ...
        return softmax
    def get_env_symbol(self, num_classes):
        """Get symbol for a3c environment
        :param num_classes: num of classes
        :return: the symbol for testing ,the relu_fc6 feature, conv_feature,
        """   
        data = mx.symbol.Variable(name='data')
        data_ref = mx.symbol.Variable(name='data_ref')
        eq_flag = mx.sym.Variable(name='eq_flag')
        ...
        softmax = mx.symbol.SoftmaxOutput(data=croped_score, normalization='valid', multi_output=True, use_ignore=True,
                                          ignore_label=255, name="softmax")
        group = mx.sym.Group([softmax, relu_fc6, warping_relu_fc6])
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            self.sym = self.get_train_symbol_video(num_classes=num_classes)
        else:
            self.sym = self.get_test_symbol_video(num_classes=num_classes)
