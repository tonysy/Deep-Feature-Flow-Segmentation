...
from segmentation.segmentation import get_segmentation_train_batch_video, get_segmentation_test_batch_video
...
class TestDataLoaderVideo(mx.io.DataIter):
    def __init__(self, segdb, config, batch_size=1, shuffle=False):
        super(TestDataLoaderVideo, self).__init__()
    ...
    def get_batch(self):
        ...
        data, label, im_info = get_segmentation_test_batch_video(segdb, self.config)

        self.data = [[mx.nd.array(data[i][name]) for name in self.data_name] for i in xrange(len(data))]
        self.im_info = im_info

class TrainDataLoaderVideo(mx.io.DataIter):
    def __init__(self, sym, segdb, config, batch_size=1, crop_height = 768, crop_width = 1024, shuffle=False, ctx=None, work_load_list=None):
        """
        This Iter will provide seg data to Deeplab network
        :param sym: to infer shape
        ...
        :return: DataLoader
        """
        super(TrainDataLoaderVideo, self).__init__()
    ...
    def get_batch_parallel(self):
        ...
        for idx, islice in enumerate(slices):
            isegdb = [segdb[i] for i in range(islice.start, islice.stop)]
            multiprocess_results.append(self.pool.apply_async(parfetch_video, (self.config, self.crop_width, self.crop_height, isegdb)))

            # temp.append(isegdb)
        # import pdb ; pdb.set_trace()
        rst = []
        for idx, multiprocess_result in enumerate(multiprocess_results):
            try:
                rst.append(multiprocess_result.get())
...
def parfetch_video(config, crop_width, crop_height, isegdb):
    # get testing data for multigpu
    data, label = get_segmentation_train_batch_video(isegdb, config)
    if config.TRAIN.ENABLE_CROP:
        data_internal = data['data']
