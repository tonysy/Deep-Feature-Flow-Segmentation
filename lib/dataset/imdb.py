...
def get_flipped_entry_outclass_wrapper_video(IMDB_instance, seg_rec):
    return IMDB_instance.get_flipped_entry_video(seg_rec)
...
class IMDB(object):
    ...
    def rpn_roidb(self, gt_roidb, append_gt=False):
        ...
        """
        if append_gt:
            print '======== Start appending ground truth annotations'
    ...
    def get_flipped_entry(self, seg_rec):
        return {'image': self.flip_and_save(seg_rec['image']),
                ...
                'width': seg_rec['width'],
                'flipped': True}
    def get_flipped_entry_video(self, seg_rec):
         return {'image': self.flip_and_save(seg_rec['image']),
                'seg_cls_path': self.flip_and_save(seg_rec['seg_cls_path']),
                'height': seg_rec['height'],
                'width': seg_rec['width'],
                ...
                'split': seg_rec['split'],
                'city': seg_rec['city'],
                'seg_id': seg_rec['seg_id'],
                'frame_id': seg_rec['frame_id']}
    def append_flipped_images_for_segmentation(self, segdb, video=False):
        """
        ...
        """
        print '=====Start appending flipped images to segdb(./lib/imdb)====='
        ...
        for i in tqdm(range(self.num_images)):
            seg_rec = segdb[i]
            if video:
                pool_result.append(pool.apply_async(get_flipped_entry_outclass_wrapper_video, args=(self, seg_rec, )))
