...
def get_segmentation_image_video(segdb, config, is_train=True):
    """
    propocess image and return segdb
    :param segdb: a list of segdb
    :return: list of img as mxnet format
    ...
    # get ref_id choice 
    id_list = range(config.TRAIN.MIN_OFFSET,0) + [0] * abs(config.TRAIN.MIN_OFFSET)
    ...
    for i in range(num_images):
        ...
        im = np.array(cv2.imread(seg_rec['image']))
        if is_train:
            # ref_id = seg_rec['frame_id'] + np.random.randint(config.TRAIN.MIN_OFFSET,config.TRAIN.MAX_OFFSET)
            ref_id = seg_rec['frame_id'] + np.random.choice(id_list)

        else:
            ref_id = seg_rec['frame_id'] + config.TEST.OFFSET

        if ref_id == seg_rec['frame_id']:
            eq_flag = 1
        ref_id = '%06d' % ref_id

        ref_image_name = seg_rec['city']+'_'+seg_rec['seg_id']+'_'+ref_id+'_leftImg8bit.png'
        ref_path = os.path.join(config.dataset.dataset_video_path, seg_rec['split'], seg_rec['city'],ref_image_name)
        assert os.path.exists(ref_path), "ref image path error, image not exists! {}".format(ref_path)
        if 'flip' in seg_rec['image']:
            ref_path = flip_and_save_func(ref_path)
        ref_im = np.array(cv2.imread(ref_path))
        # print 'orginal:',seg_rec['image']
